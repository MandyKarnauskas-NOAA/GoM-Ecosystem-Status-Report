...
plotIndicatorTimeSeries <-  function(filename, coltoplot=2, sublabel=F, plotrownum = 1, plotcolnum = 1, 
                                     yposadj=1, widadj=1, trendAnalysis=T, redgreen=T, outname=NA, sameYscale=F)  {
...
if (is.na(outname))  {  filnam <- paste(c(unlist(strsplit(filename, ".csv"))), ".eps", sep="") }   else   {  filnam <- outname  }
...
if (length(tim) >= 5 & redgreen==T) {
  ...
  if (mean(diff(tim_all)) <= 1.1 )  {  lines(tim_all, co_all, lwd=2); points(tim_all, co_all, pch=20)   }                       # plot time series 
  ...
  points(1, 1.225, pch=20, cex=6)                                                 # analyze mean of last 5 years
  ...
  if (monthly == F)  {  res   <- summary(lm(last5~tim[1:5]))   } else {    res   <- summary(lm(last5~tim[1:60]))   }
    ...
    slopelim <- (1.0/ (length(last5))) * sd(co, na.rm=T)
      if (slope >  slopelim)  { arrows(0.98, 0.89, x1 = 1.02, y1 = 1.01, length = 0.1, angle = 45, code = 2, lwd = 3)  }  
      if (slope <  -slopelim) { arrows(0.98, 1.01, x1 = 1.02, y1 = 0.89, length = 0.1, angle = 45, code = 2, lwd = 3) }  
      if (slope <= slopelim & slope >= -slopelim) { arrows(0.97, 0.95, x1 = 1.03, y1 = 0.95, length = 0.1, angle = 45, code = 3, lwd = 3) } 
...
dev.copy2eps(filnam, width=((wid+10)/7)*plotcolnum2, height=(3.5*plotrownum2))  #, pointsize=12, res=72*4)
