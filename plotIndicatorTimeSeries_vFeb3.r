...
#  ylabadj    = manual adjustment of position of y-axis label                  #
...
#  plotIndicatorTimeSeries("menhaden_abundance_index.csv", coltoplot=2, plotrownum=1, plotcolnum=1, sublabel=F, ylabadj=0.8, widadj=0.8)
...
plotIndicatorTimeSeries <-  function(filename, coltoplot=2, sublabel=F, plotrownum = 1, plotcolnum = 1, 
                                     adjlabmain=1, adjlaby=1, ylabadj=1, widadj=1, trendAnalysis=T, outname=NA)  {
...
if (length(tim) > 5) {  
  ...
  plot(tim_all, co_all, col=0, axes=F, xlab="", ylab="", main="")                       # plot time series
...
if (length(tim) >= 5) {
  ...
  polygon(c(max(tim_all)-4.5-as.numeric(monthly)/2.1, max(tim_all)+0.5-as.numeric(monthly)/2.4, max(tim_all)+0.5-as.numeric(monthly)/2.4, max(tim_all)-4.5-as.numeric(monthly)/2.1), 
  ...
  lines(tim, co, lwd=2); points(tim, co, pch=20)                         # plot time series 
  ...
  axis(2, las=2); box()
#  if (diff(range(co, na.rm=T)) > 0.99) { ladj <- 2.5 }                                     # adjustment for location of y-axis label
#  if (diff(range(co, na.rm=T)) > 99) { ladj <- 3 } 
#  if (diff(range(co, na.rm=T)) > 999 | diff(range(co, na.rm=T)) <= 0.99) { ladj <- 3.5 } 
#  if (diff(range(co, na.rm=T)) > 9999) { ladj <- 4 } 
  if (adjlaby==1)  {  mtext(side=2, line=3*ylabadj, d1[2,i], cex=1.2/mean(plotcolnum2, plotrownum2))  }  else  {       # y-axis label
                      mtext(side=2, line=3*ylabadj, d1[2,i], cex=1.2*adjlaby)  }
...
#  if (diff(range(co, na.rm=T)) > 0.99) { ladj <- 2.5 }                                     # adjustment for location of y-axis label
#  if (diff(range(co, na.rm=T)) > 99) { ladj <- 3 } 
#  if (diff(range(co, na.rm=T)) > 999 | diff(range(co)) <= 0.99) { ladj <- 3.5 } 
#  if (diff(range(co, na.rm=T)) > 9999) { ladj <- 4 } 
  if (adjlaby==1)  {  mtext(side=2, line=3*ylabadj, d1[2,i], cex=1.2/mean(plotcolnum2, plotrownum2))  }  else  {       # y-axis label
                      mtext(side=2, line=3*ylabadj, d1[2,i], cex=1.2*adjlaby)  }
