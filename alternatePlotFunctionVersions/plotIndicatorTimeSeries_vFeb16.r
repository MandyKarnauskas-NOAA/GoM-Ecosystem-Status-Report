...
plotIndicatorTimeSeries <-  function(filename, coltoplot=2, sublabel=F, plotrownum = 1, plotcolnum = 1, 
                                     adjlabmain=1, adjlaby=1, yposadj=1, widadj=1, arradj=1, trendAnalysis=T, redgreen=T, outname=NA, sameYscale=F)  {
...
if (length(tim) > 5) {  
  ...
  par(mgp=c(2.5*yposadj,1,0), xpd=T)
  ...
  par(xpd=F)
#  if (adjlabmain==1)  {  mtext(side=3, line=1, mm, font=2, cex=1/plotcolnum)  }  else  {    # main label                  , cex=1.5/mean(plotcolnum2, plotrownum2)
#                         mtext(side=3, line=1, mm, font=2, cex=1*adjlabmain)  } 
...
if (length(tim) >= 5 & redgreen==T) {
  ...
  axis(2, las=2); box()
#  if (adjlaby==1)  {  mtext(side=2, line=3*yposadj, d1[2,i], cex=1/plotrownum)  }  else  {       # y-axis label          , cex=1.2/mean(plotcolnum2, plotrownum2)  
#                      mtext(side=2, line=3*yposadj, d1[2,i], cex=1*adjlaby)  }
...
#    iw <- 10 /wid / plotcolnum * arradj
      if (slope >  slopelim)  { arrows(0.98, 0.89, x1 = 1.02, y1 = 1.01, length = 0.1, angle = 45, code = 2, lwd = 3)  }  #  add.image(1,0.9, uparrow, col=grey((0:256)/256), image.width = iw, image.height = iw)   }
      if (slope <  -slopelim) { arrows(0.98, 1.01, x1 = 1.02, y1 = 0.89, length = 0.1, angle = 45, code = 2, lwd = 3) }   #add.image(1,0.9, dnarrow, col=grey((0:256)/256), image.width = iw, image.height = iw)  }
      if (slope <= slopelim & slope >= -slopelim) { arrows(0.97, 0.95, x1 = 1.03, y1 = 0.95, length = 0.1, angle = 45, code = 3, lwd = 3) } # add.image(1,0.9, sdarrow, col=grey((0:256)/256), image.width = iw, image.height = iw)  }
            }
         }
...
dev.copy(pdf, filnam, width=((wid+10)/7)*plotcolnum2, height=(3.5*plotrownum2))  #, pointsize=12, res=72*4)
